...
/** 로그에만 남기는 내부 상세. 외부 응답에 나가지 않는다. */
export type ErrorContext = Record<string, unknown>;

/** 외부 응답에 담아도 되는 상세. 값·식별자를 담을 수 있으나 비밀정보는 금지. */
export type PublicDetails = Record<string, unknown>;

export interface AppErrorOptions {
  /** 개발환경 응답과 서버 로그에 쓰이는 상세 메시지 */
  readonly message?: string;
  /** 외부 응답에 포함할 상세 (운영환경에서도 노출됨) */
  readonly details?: PublicDetails;
  /** 사용자에게 줄 해결 안내 */
  readonly hint?: string;
  /** 서버 로그 전용 컨텍스트 — 외부 응답에 절대 포함되지 않음 */
