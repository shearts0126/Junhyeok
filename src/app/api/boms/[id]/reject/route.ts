...
import { assertNoQueryParams, rejectBom, parseRejectBomInput } from '@/modules/bom/application';
import { withErrorHandling } from '@/shared/errors';
import { createSupabaseServerClient } from '@/shared/supabase';

/**
 * `POST /api/boms/{id}/reject` — 반려 `bom.approve`.
 *
...
export async function POST(
  request: Request,
  context: RouteContext<'/api/boms/[id]/reject'>,
  ...
  return withErrorHandling(
    ...
    async (requestId) => {
      ...
      assertNoQueryParams(new URL(request.url).searchParams, '반려는 파라미터를 받지 않습니다.');
      ...
      const input = parseRejectBomInput(body);
      const result = await rejectBom(actor, id, input);
