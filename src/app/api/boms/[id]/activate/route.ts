...
import { assertNoQueryParams, activateBom, parseActivateBomInput } from '@/modules/bom/application';
...
export async function POST(
  request: Request,
  context: RouteContext<'/api/boms/[id]/activate'>,
  ...
  return withErrorHandling(
    ...
    async (requestId) => {
      ...
      assertNoQueryParams(new URL(request.url).searchParams, '활성화는 파라미터를 받지 않습니다.');
      ...
      const input = parseActivateBomInput(body);
      const result = await activateBom(actor, id, input);
