...
import { assertNoQueryParams, archiveBom, parseArchiveBomInput } from '@/modules/bom/application';
...
export async function POST(
  request: Request,
  context: RouteContext<'/api/boms/[id]/archive'>,
  ...
  return withErrorHandling(
    ...
    async (requestId) => {
      ...
      assertNoQueryParams(new URL(request.url).searchParams, '보관는 파라미터를 받지 않습니다.');
      ...
      const input = parseArchiveBomInput(body);
      const result = await archiveBom(actor, id, input);
