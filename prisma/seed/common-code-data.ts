 ...
 *
 * ## 원본과 지시 수량의 차이 (시드 검증 보고 대상)
 *
 *   | 그룹 | 지시 | 원본 실측 | 사유 |
 *   |---|---|---|---|
 *   | MAJOR_CATEGORY | 13 | **12** | 원본 사전에 12행뿐. 문서의 "13"은 집계 오기 |
 *   | MATERIAL_CATEGORY | 39 | **38** | 원본 39행 중 `ET`(Etc/기타)가 2회 등장(순번 24·30). UNIQUE(group, code) 위반이라 뒤의 1건 제외 |
 *
 *   따라서 합계는 100 이 아니라 **98** 이다. 임의 코드를 만들어 수를 맞추지 않는다.
 *
 * ## 기타 원본 이상값 (보정하지 않고 그대로 시드)
 *
 *   - 보관처 `BOC` 와 `BON` 의 명칭이 동일하다(본코스메틱) — 중복 의심값
 *   - 대분류에 채널성 코드(OY·DS·MS)가 섞여 있다 → `attributes.isChannelCategory`
