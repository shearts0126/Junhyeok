...
- 상태: **검증용 수집기(READY_FOR_REVIEW). 공식 명세 원문 미확인, 실수집 미검증, 정기 수집 등록·실행 차단.** 설계 담당자가 첫 외부 연동 검증 대상으로 지정한 소스이며, API 이용 가능성이나 실제 공급자 적합성을 확정했다는 뜻이 아니다.
- 명세 확인 수준(수집기 메타데이터 `specStatus`): `SNIPPET_ONLY`. 근거 목록은 `SPEC_EVIDENCE`(`koreaexim-fx.ts`)에 기록. 다섯 단계 **구현의 존재**와 **실제 공급자 적합성**은 별개이며, 후자는 원문 확인·허용 네트워크·실제 인증·실응답 파싱·대조 검증이 모두 끝나야 성립한다. 인증키 하나로 끝나지 않는다.
- 차단 위치: 파이프라인 정기 게이트(`SCHEDULED` 모드 → 외부 요청 전 `FAILED/SCHEDULED_REQUIRES_CONFIRMED_SPEC`), enqueue 게이트(`--mode SCHEDULED` → `NOT_SCHEDULABLE`, 종료 코드 7, DB 작업 없음), `isSchedulable()=false`. 정기 실행 설정(`fin_collection_schedules`)은 구조만 있고 활성화된 행이 없다. 실행은 `--mode VERIFICATION` 으로만 가능하며 결과는 최신 관측·집계에 반영되지 않는다.
...
## 2a. 근거가 없어 확정하지 않은 항목(추정 구현 금지, `SPEC_EVIDENCE.unverified`)

| 항목 | 현재 처리 | 확정에 필요한 것 |
|---|---|---|
| 환율 종류의 정의(ttb/tts/deal_bas_r/bkpr/kftc_*) | 종류별로 원문 값을 그대로 보존. 어느 값을 환산에 쓸지 미결정 | 공식 원문의 필드 정의 + 설계 담당자 결정 |
| 통화 단위 규칙(JPY(100) 등) | `cur_unit` 원문 보존, 값 단위 해석 없음 | 공식 원문 |
| 기준일 필드 | 응답 필드 확인 안 됨. 요청 searchdate 를 `asOfDateBasis='REQUESTED'` 로 보존 | 실응답 원문 |
| 빈 응답의 의미 | 0건 성공으로만 기록, 전 영업일 값 대체 없음 | 공식 원문(비영업일·11시 이전 응답 형태) |
| 결과 코드 형태 | 첫 원소의 `result` 로만 판정 | 오류 응답의 실제 형태(배열/객체, 원소마다 오는지) |
| 숫자 형식(콤마·소수 자릿수) | 콤마 제거 후 십진 문자열 보존 | 공식 원문 |

## 3. 검증 결과 구분

| 구분 | 결과 |
|---|---|
| 가상 응답 기반 내부 시험(`test/koreaexim-fx.test.ts` 5건, 검증 모드) | 통과: 정기 실행·정기 enqueue 차단(외부 요청 0건), 3통화×6종류=18건 정규화·대조·저장, 인증키 미잔존, 재수집 무증가, 빈 배열 0건, result 코드 분기, HTTP 오류 분류, 다중 일자 미구현 표시. **파이프라인 규칙 검증일 뿐 공급자 적합성 검증이 아니다** |
| 공식 명세 원문 | **미확인**(본 환경 두 호스트 차단). 발췌 기준 |
| 실수집 | **미검증.** 인증키 없음, 본 환경 egress 차단(`oapi.koreaexim.go.kr` 프록시 403 확인) |

## 4. 정기 수집 해제 조건(전부 필요, 순서대로)

1. 공식 명세 원문 확인: 공식 페이지·이용약관·응답 정의를 원문으로 열람하고 §2a 항목을 확정, `SPEC_EVIDENCE` 갱신.
2. 허용된 네트워크: 승인된 실행 환경에서 `oapi.koreaexim.go.kr` egress 허용 확인.
3. 인증: 회사 명의 인증키 1종을 그 환경의 `FIN01_KOREAEXIM_AUTHKEY` 로 설정(대화·저장소에 넣지 않음). 다른 환율 키를 동시에 요구하지 않는다.
4. 검증 모드 실수집: `pnpm enqueue --collector koreaexim-fx --mode VERIFICATION --from <영업일> --to <같은 날> --request-id fx-verify-1` 후 worker 처리, 실응답 파싱·대조 결과와 원문 차이를 제출.
5. 위 결과를 설계 담당자가 검토한 뒤에만 `specStatus` 를 `OFFICIAL_TEXT_CONFIRMED` 로 바꾸고(코드 변경·검토 필요) 정기 등록을 논의한다. 이 문서 기준으로는 아직 어느 단계도 완료되지 않았다.

현재 환율 데이터는 은행 현금 환산·실제 보고에 연결하지 않았다. 첫 실수집이 원문 명세와 다르면 대체 공급자 검토 여부를 보고하되, 현재는 대체가 필요하다고 판단할 근거가 없다.
