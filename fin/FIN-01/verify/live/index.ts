...
export async function runLive(source: LiveSource, targetDate: string): Promise<LiveResult> {
  switch (source) {
    case 'fx-exim': {
      ...
      if (!r.ok) return blockedNoCredentials(source, r.missing, LIVE_IMPLEMENTATION[source]);
      ...
      return fetchSummary(source, url, LIVE_IMPLEMENTATION[source], { redactQuery: true });
    }
    case 'fx-ecos': {
    ...
    case 'bank-kftc': {
      ...
      return fetchSummary(source, url, LIVE_IMPLEMENTATION[source], {
