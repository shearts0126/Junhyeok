...
- 검증 환경 제약(전 출처 공통): 본 실행 환경의 네트워크 egress 정책이 외부 호스트 전부를 차단했다(프록시 403, `x-deny-reason: host_not_allowed`). 공식 문서 원문을 직접 열람하지 못했고, 실제 API 호출도 불가능했다. 아래 "공식 근거" 는 **공식 도메인 문서의 검색 결과 발췌**로 확인한 것이며, 원문 열람으로 재확인이 필요하다. 어떤 출처도 자격정보(토큰·키·계정)가 제공되지 않았다. 따라서 **실제 수집 검증 완료 상태인 출처는 없다.**
...
| 출처 | 검증 상태 | 한 줄 결론 |
|---|---|---|
| 은행 A: 금융결제원 오픈뱅킹 | 공식 문서 확인(발췌) → 접근 대기 | 법인 이용 2025-01-02 확대(금융위 보도). 이용기관 등록·보안점검(약 4주)·은행별 조회대상 계좌등록 필요. 거래별 고유 ID 없음. USD 계좌 지원 미확인 |
...
| 광고: 구글 | 공식 문서 확인(발췌) → 접근 대기 | GAQL `metrics.cost_micros`/`segments.date`/`customer.currency_code`/`customer.time_zone`. 2026-09-09 개발자 토큰 폐지→Google Cloud 프로젝트 기반 접근, 2026-08-05부터 신규 리프레시 토큰에 패스키 필요(공식 블로그 발췌). 잔액 조회 미확인 |
...
| 국민·기업·신한·우리·하나 지원 | 5개 은행 모두 오픈뱅킹 참가은행으로 널리 알려져 있으나 참가기관 목록 페이지(https://openapi.kftc.or.kr/service/openBanking) 원문 미열람 → 재확인 필요 | "전 은행 원화 계좌" 표기(발췌). 은행별 개별 확인 미실시 | "20개 은행" 지원 표기(발췌). 5개 은행 개별 확인 미실시 |
| 법인 계좌 | 2025-01-02부터 법인 이용자 확대(금융위·금결원 발표 발췌). 각 은행에서 "조회 대상 계좌등록" 사전 신청 필요 | 법인 CMS 상품(중견·대기업용) | 기업(법인·개인사업자) API 별도 제공 |
| USD(외화) 계좌 | **미확인**(공식 문서에서 외화계좌 조회 지원 여부를 찾지 못함) | 17개 은행 외화계좌 잔액·거래내역·고시환율 지원 표기(발췌) | 기업 외화 거래내역 API 별도 존재(발췌) |
| 거래내역 | 거래내역조회 API, 페이지당 최대 25건(발췌), `next_page_yn`/`befor_inquiry_trace_info` 페이징 | 지원 표기. 필드 미확인 | 지원. 필드 미확인 |
| 전일 잔액 | 잔액조회 API 는 조회 시점 잔액. 일자 기준 잔액은 거래내역의 `after_balance_amt` 로 유도 필요 | 미확인 | 미확인 |
| 거래 고유 ID | **없음**(`bank_tran_id` 는 API 호출 단위). 대체키 필요 | 미확인 | 미확인 |
...
비교 근거 요약(설계 담당자 선택용): A 는 공식 표준·법인 확대가 확인됐지만 USD·거래 ID·등록 절차(약 4주)가 부담이다. B·C 는 외화 지원이 표기되어 있으나 필드·요금·인증 갱신을 확인하지 못했다. 세 후보 모두 실제 응답을 확보하지 못했으므로 기본 계약을 지정하지 않는다.
...
| 공식 근거 | https://developers.facebook.com/docs/marketing-api/insights/ · https://developers.facebook.com/docs/marketing-api/insights/best-practices/ · https://developers.meta.com/blog/updates-to-ads-management-standard-access-feature/ | https://developers.google.com/google-ads/api/docs/get-started/onboarding · https://ads-developers.googleblog.com/2026/02/an-update-on-google-ads-api-developer.html · https://ads-developers.googleblog.com/2026/07/passkey-authentication-requirement-for.html · https://developers.google.com/google-ads/api/docs/api-policy/developer-token | https://naver.github.io/searchad-apidoc/ (차단) · https://github.com/naver/searchad-apidoc (발췌) · StatReport 공지 http://naver.github.io/searchad-apidoc/notice/2021/03/09/notice1/ | https://ads.coupang.com/AaaAna.html (광고분석가이드, 발췌) · https://developers.coupang.com/ko/api (판매자 Open API, 발췌) | https://business-api.tiktok.com/portal/docs (차단) · https://github.com/tiktok/tiktok-business-api-sdk (발췌) |
...
| 인증·갱신 | System User 토큰(장기). Marketing API 접근 등급 + 비즈니스 인증 필요 | OAuth 리프레시 토큰. **2026-09-09 개발자 토큰 폐지, Google Cloud 프로젝트에 접근 등급 귀속**. 2026-08-05부터 신규 리프레시 토큰 발급에 패스키 필수 | 액세스라이선스·비밀키·CUSTOMER_ID, `X-Timestamp`/`X-API-KEY`/`X-Customer`/`X-Signature`(HMAC-SHA256). 광고플랫폼 > 도구 > API 사용관리 | — | Access Token(장기). 만료 정책 미확인 |
...
| 다음 조치 | 광고 계정 목록·법인 매핑, 앱·System User 토큰 준비 | Google Cloud 프로젝트·API 접근 신청, 패스키로 리프레시 토큰 발급, 계정 목록 | API 라이선스 발급, CUSTOMER_ID 목록 | 쿠팡 광고 담당자에게 판매자/대행사 API 제공 여부 문의. 없으면 엑셀 보조 경로로 확정 | 앱 생성·광고주 인가, 계정 목록 |
