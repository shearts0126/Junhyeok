...
| **6** | **자가승인 설정 분리** — 전역 `allow_self_approval` → **`allow_self_approval_sku`** / **`allow_self_approval_bom`** 최소 2개로 분리. 재고조정·음수재고 예외·월마감 해제는 설정과 무관하게 **항상 금지** | `03 §7`, `05 §10.1`, `00 §15 D-07` | `T0-7`, `T1-4`, `T3-9` | **`T0-7` 착수 전** |
