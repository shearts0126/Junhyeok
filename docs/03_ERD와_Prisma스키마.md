...
> ✏️ **2026-08-25 설계복구 (Warehouse, T08)**: 위 ERD 의 `default_location_id FK "NOT NULL 강제"` 가 정본이고, 아래 `model Warehouse` 의 **`defaultLocationId String?`(nullable) 선언은 `SUPERSEDED BY 19_설계복구_Warehouse.md §W-D5`** 다 — 두 초안이 서로 모순되며 `00 §2.2 G-05` 가 NOT NULL 을 명시하므로 **NOT NULL 이 정본**이다(PRE-FLIGHT 의 nullable 유지 권고는 **REJECTED**). 이에 따라 T08-1 에서 다음이 추가된다 — ① `warehouse` 에 `@@unique([warehouseId, id])`(정확히는 `@@unique([id, ...])` 가 아니라 `warehouse_location (warehouse_id, id)` 쪽의 보조 UNIQUE)를 두고 ② `default_location_id` FK 를 **`(id, default_location_id) REFERENCES warehouse_location (warehouse_id, id) DEFERRABLE INITIALLY DEFERRED`** composite FK 로 걸어 **"창고의 DEFAULT 로케이션은 반드시 그 창고의 로케이션"** 을 DB 가 강제한다(**§W-D6**). 순환 참조는 §W-D7 의 pre-generated UUID 순서로 해소하며 **사후 UPDATE 는 없다**. `supplier_id` 는 T08 시점에 **one-way CHECK `(supplier_id IS NULL OR warehouse_type = 'SUPPLIER_SITE')`** 만 걸고, 양방향 IFF 는 마이그레이션 Phase 7(`T4-19`) 완료 후에 추가한다(**§W-D13**). `ledgerEntries`·`balances` relation 은 **T09 이전에는 만들지 않는다**(**§W-D18**). FK 의 `onDelete`/`onUpdate` 는 **§W-D19** 가 확정한다.
