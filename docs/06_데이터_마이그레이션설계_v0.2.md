...
> ✏️ **2026-08-13 설계복구 (BOM, T07)**: Phase 8 이관 계약은 그대로 유지하되 구현 계약은 **`18_설계복구_BOM.md`** 를 따른다 — 매핑표에 **`uom` 열이 없으므로** `bom_line.uom := 구성품 SKU 의 baseUom` 으로 채운다(**§D-11**). `bom_header.version='1.0'` · `effective_from=cutover_date` · 전량 `DRAFT` 는 **§D-4·§D-5·§D-6** 과 정합한다. 활성화 차단 게이트는 **`isRequired=true` 라인**에만 적용된다(**§D-10**). 마이그레이션은 **T07 구현과 분리**되어 R1a-4 `T4-19` 에서 실행한다(**§D-32**).
...
> ✏️ **2026-08-25 설계복구 (Warehouse, T08)**: 위 15종 표는 **정본이며 `19_설계복구_Warehouse.md §W-D37` 이 그대로 T08-2 seed 계약으로 채택**한다(코드·명칭·유형 동일, `timezone` 전부 `Asia/Seoul`, `active` 전부 `true`, seed 는 idempotent, ⛔ 임의 추가·삭제 0). `externalSystemId` 는 **전부 `null`** 이다(**§W-D14**) — 저장소에 `ExternalSystem` seed 가 존재하지 않아 어떤 창고에도 실재하는 외부시스템을 연결할 수 없기 때문이며, ⛔ 추정 값을 만들지 않는다. **`SUPPLIER_SITE`는 `supplier_id` FK 연결 (Phase 7 거래처 이관 후 연결, 또는 Phase 3에서 거래처 선등록)** 두 선택지는 `CLARIFIED BY §W-D13` 이다 — **Phase 3 선등록은 채택하지 않는다.** 거래처 정본이 Phase 7 source 에만 있으므로 Phase 3 에서 선등록하려면 `supplierCode` 를 **발명**해야 하고, 그것은 나중에 진짜 거래처가 들어올 때 중복·오연결을 만든다. 따라서 **staged-link lifecycle** 을 채택한다 — ① Phase 3(=T08-2 seed): 창고 15종을 만들되 11개 `SUPPLIER_SITE` 는 `supplier_id = NULL`(문서상 상태 `UNLINKED_PENDING_SUPPLIER_MIGRATION`, ⛔ DB enum/컬럼 아님) ② T08 시점 DB 제약은 **one-way CHECK `(supplier_id IS NULL OR warehouse_type = 'SUPPLIER_SITE')`** 하나뿐이다(**§W-D13**) ③ Phase 7(`07` v0.2 `T4-19` — 공통코드·사용자·창고·SKU·바코드·매핑·**거래처**·BOM) 에서 11건을 **exact 1:1** 로 연결한다 — **0 match → block · 2+ match → block** ④ 링크가 전부 해소된 뒤에 비로소 **양방향 IFF CHECK** 를 추가한다(**§W-D13 최종 운영 불변식**). ⛔ 축약어(`BOC`·`IJC`·`CSM`·…)를 `Supplier.supplierCode` 로 자동 승격하지 않고, ⛔ 이름 유사성만으로 FK 를 연결하지 않으며, ⛔ fake Supplier 를 만들지 않는다. **BOC/BON(Q-02)** 은 T08 blocker 가 아니다 — 창고 identity 는 `SUP_BOC`·`SUP_BON` **2개로 보존**하고 동일 업체 여부는 Phase 7 source evidence 로 결정한다(**§W-D39**).
