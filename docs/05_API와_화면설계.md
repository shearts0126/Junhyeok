...
> ✏️ **2026-08-25 설계복구 (Warehouse, T08)**: 위 5행은 원문으로 보존하되 다음이 **`19_설계복구_Warehouse.md`** 로 supersede/clarify 되었다 — ① 두 GET 행의 권한 **`전체`** 는 `SUPERSEDED BY §W-D22` 다. `§11.22` 화면별 권한표(`창고·로케이션` 행)가 더 좁고, 같은 문서 안에서 권한표가 이길 때 그것을 채택해 온 선례(`13 §11` 외부매핑 `E = —`)를 따른다 — 권한은 신규 **`warehouse.read` / `warehouse.create` / `warehouse.update`** 정확히 3종이고 ⛔ `warehouse.delete` 는 만들지 않는다(물리삭제 금지). ② `PATCH` 행의 **`재고 존재 시 비활성 차단`** 은 `DEFERRED BY §W-D27` 다 — 재고 모델(T09)이 없으면 이 안전장치를 구현할 수 없고 `hasInventory=false` 같은 상수 가정은 안전장치를 무효화하므로, **`active` mutation 자체를 `T2-20` 으로 연기**한다(선례: `sku/application/workflow.ts` 의 BOM usage provider 부재 → T1-4B 연기, "상수 가정 금지·라우트 stub 금지"). T2-1 단계의 PATCH 는 `active` 를 **받지 않으며 보내면 400** 이다(⛔ 조용한 무시 없음). ③ `CreateWarehouseDto` 는 이름만 있었고 PATCH 요청 열은 `—` 였다 — **§W-D24·§W-D25·§W-D26** 이 필드까지 확정한다(`warehouseCode`·`warehouseType` 은 create-only immutable). ④ `GET /api/warehouses` 의 응답·필터·페이지네이션 계약은 **§W-D30·§W-D31**(기존 list envelope `{items,page,pageSize,total,totalPages}` 재사용, `pageSize` 서버 고정 50, 미지원 파라미터는 400), 로케이션 2종은 **§W-D32·§W-D33·§W-D34** 가 확정한다(sub-resource 응답은 기존 관례대로 `{...result, requestId}`). ⑤ `POST /api/warehouses` 의 **DEFAULT 로케이션 자동 생성**(§00 G-05)은 유지되며 실행 순서만 **§W-D7** 로 clarify 된다(pre-generated UUID · deferred composite FK · 사후 UPDATE 없음). `DEFAULT` 는 예약 코드라 `POST .../locations` 로 만들 수 없고(**§W-D9**) 변경·비활성도 불가하다(**§W-D10**). ⑥ `IN_TRANSIT` 창고는 시스템 예약 singleton 이며 public mutation 대상이 아니다(**§W-D11·§W-D12**). ⑦ `Warehouse[]` 응답에 `supplierId` 를 그대로 노출하지 않는 projection 경계는 **§W-D17** 이 확정한다.
