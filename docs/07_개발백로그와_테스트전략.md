...
> ✏️ **2026-08-13 설계복구 (SKU 상세 ⑥ 공급조건 탭, T1-6B4)**: 위 `T03-6 SKU 상세 화면 8탭` 의 ⑥ 공급조건 탭을 **`16_설계복구_SKU상세잔여탭.md` §41~§63** 으로 확정해 구현했다(§40 이 "T06 이후"로 미뤄 둔 차단 사유가 T06-1~T06-4 완료로 해소됐다). 원문 행은 그대로 보존하며 요점은 — ① 이 탭은 **read-only summary** 이고 공급조건·가격 mutation owner 는 계속 T06-4 `/master/suppliers` 다(`거래처 관리에서 보기` 링크만 제공). ② **현재 유효한 공급조건만** 표시하며 과거·미래 이력은 관리화면의 책임이다. ③ `최근 단가` 는 **서버 업무일자 기준 승인 유효가격**이며(T06-3 계약 재사용) 미승인 가격은 반영되지 않는다. ④ 이를 위해 supporting API `GET /api/skus/{id}/supplier-skus` 를 **정확히 1개** 추가했다 — SKU 역조회 경로가 없었고, 이 API 가 요약과 최근 단가를 한 번에 반환해 N+1 을 만들지 않는다(batch price resolver). ⑤ 탭 노출은 `supplier.read` **AND** `supplier_price.read` 이며 EXECUTIVE 에게는 숨겨진다. ⑥ 상세는 이제 **7탭**(⑦ BOM 은 T07 이후)이고 등록 화면은 3탭 그대로다. ⑦ 검증은 기존 stack 그대로 — unit·DB integration·Playwright E2E 이며 schema·migration·신규 permission·seed 변경은 **0** 이다.
...
> ✏️ **2026-08-13 설계복구 (BOM 전체, T07)**: **T07 PRE-FLIGHT 는 `BLOCKED — DESIGN RECOVERY REQUIRED`** 였다. BOM 스키마 필드는 `03 §Layer2` 에 완비돼 있었으나 ① **활성 BOM 교체 규칙이 `05:129`(기존 ACTIVE 자동 INACTIVE)와 `03:EXCLUDE`(ACTIVE 기간 중첩 거부) 사이에서 직접 충돌**하고 ② `bom_line` 중복 방지 UNIQUE 가 `alternate_group` NULL 로 무력화되며 ③ cycle 알고리즘·UOM·전개 공식·aggregation·SupplierSku 선택·provisional·currency·VAT·concurrency·DTO 6종·permission 이 전부 미정이었다. 계약은 **`18_설계복구_BOM.md`(D-1 ~ D-32)** 로 확정한다. 위 8개 작업 ID(`T07-1`~`T07-8`)는 authoritative 로 유지하며, `T07-7` 만 `T07-7A`(단일 레벨 원가) / `T07-7B`(multi-level roll-up)로 **implementation subdivision** 한다(backlog ID 대체 아님). `T07-6` 의 v0.2 추가분 `최대 조립가능수량`은 **현재고 의존이므로 R1a-2 이후**로, `T07-8` 의 v0.2 추가분 `BOM 동기 업로드`는 **`PENDING #7` 확정 전까지** 유예한다. **`T1-6B5`(SKU 상세 ⑦ BOM 탭)는 `T07-3` 완료 직후 read-only 로 삽입**한다 — mutation owner 는 standalone `/master/boms` 다(T05-4A·T06-4 와 같은 원칙). 완료조건의 `구 버전 자동 INACTIVE`(E2E-06)는 **§D-7** 에 따라 `구 버전 적용기간 자동 마감` 으로 대체되며, 완료조건 `박스 단가 = 가격÷입수량`(T07-7)은 **§D-19** 에 따라 `quantityPer` 에 이미 반영된 결과이지 별도 나눗셈이 아니다.
...
> ✏️ **2026-08-25 설계복구 (Warehouse, T08)**: **T08 PRE-FLIGHT 는 `BLOCKED — DESIGN RECOVERY REQUIRED`** 였다(12 blocker). 계약은 **`19_설계복구_Warehouse.md`(W-D1 ~ W-D42)** 로 확정한다. 작업 분해는 **v0.2 `T2-1` 이 정본**이며 legacy 번호는 alias 로만 유지한다(**§W-D1**) — **`T08-1` = `T2-1A`**(스키마 + warehouse relation landing) · **`T08-2` = `T2-1B`**(application/API + DEFAULT location 트랜잭션 + 15종 seed). ~~**`T08-3` 창고 화면**~~ 은 **`SUPERSEDED BY` v0.2 `T2-20`** 이다 — v0.2 가 화면을 `T2-20`(현재고 화면과 같은 작업)으로 옮겼고, 그 이유는 위 완료조건 **`재고 존재 시 비활성 차단`** 이 current-stock capability(`T2-2` 이후)를 요구하기 때문이다. 재고 모델 없이 구현하면 `hasInventory=false` 같은 **상수 가정**이 필요해져 안전장치 자체가 무효화된다(선례: `src/modules/sku/application/workflow.ts` 가 BOM usage provider 부재를 이유로 `archive` 를 T1-4B 로 연기하며 **"상수 가정 금지·라우트 stub 금지"** 를 명시했다). 따라서 T08 범위에서 **UI 는 0줄이고 `active` mutation 도 만들지 않는다**(**§W-D27·§W-D28**). `T08-1` 완료조건이 비어 있던 자리는 **§W-D5·§W-D6**(`defaultLocationId` **NOT NULL** + same-warehouse composite FK `DEFERRABLE INITIALLY DEFERRED`)이 채우고, `T08-2` 의 `생성 직후 defaultLocationId NOT NULL` 은 **§W-D7** 의 pre-generated UUID 순서로 달성한다(사후 UPDATE 없음). `SUPPLIER_SITE` 11개의 거래처 연결은 이 Epic 이 아니라 **마이그레이션 Phase 7**(v0.1 `T15-9` "Phase 7~8 거래처·공급조건·BOM" / v0.2 `T4-19` "Phase 1~8") 소유다(**§W-D13·§W-D38**).
